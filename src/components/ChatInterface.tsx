...
export function ChatInterface({
  ...
  const inputBg = isDark ? "bg-[#1e2433] border-[#2d3548]" : "bg-white border-[#d8d0c4]";
  const userBubble = isDark ? "bg-[#2979ff] text-white" : "bg-[#b5651d] text-white";
  const aiBubble = isDark ? "bg-[#252d40] text-[#e8eaf0] border border-[#2d3548]" : "bg-white text-[#2c2416] border border-[#e0d8cc]";
  const sourceLink = isDark ? "text-[#5b9cf6]" : "text-[#b5651d]";
  const copyBtn = isDark ? "bg-[#2d3548] text-[#6b7a99] hover:text-white" : "bg-[#ede8e0] text-[#8a7d6b] hover:text-[#2c2416] border border-[#d8d0c4]";

  return (
    <div className={clsx("flex flex-col h-full", bg)}>

      {/* Chat header */}
      <div className={clsx("px-4 py-3 border-b flex items-center justify-between flex-shrink-0", border)}>
        <span className={clsx("text-xs font-bold uppercase tracking-widest", subtext)}>Chat</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 space-y-3">
            ...
            <div className={clsx(
              ...
              {msg.role === "assistant" && (
                <button
                  ...
                  className={clsx("absolute -top-2 -right-2 w-6 h-6 rounded-lg flex items-center justify-center opacity-0 group-hover/msg:opacity-100 transition-all shadow-sm", copyBtn)}
                  title="Copy"
                >
              ...
              <div className="mt-1.5 max-w-[80%] space-y-1">
                {msg.sources.map((source, idx) => (
                  <button
                    ...
                    className={clsx("flex items-center gap-1 text-xs transition-opacity hover:opacity-70", sourceLink)}
                  >
                    <ArrowUpRight className="w-3 h-3 flex-shrink-0" />
                    <span>Source: Page {source.page_number} — {source.file_name}</span>
                    {source.confidence && source.confidence > 50 && (
                      <span className={clsx("ml-1 text-[10px] opacity-60")}>{source.confidence}%</span>
                    )}
                  </button>
                ))}
              </div>
        ...
        {isLoading && (
          <div className="flex items-start">
            <div className={clsx("px-4 py-3 rounded-2xl rounded-tl-sm text-sm max-w-[80%]", aiBubble)}>
              {streamingContent ? (
                <div className={clsx("prose prose-sm max-w-none", isDark ? "prose-invert" : "")}>
      ...
      <form onSubmit={handleSubmit} className={clsx("px-4 py-3 border-t", border)}>
        <div className={clsx("flex items-center gap-2 px-4 py-2.5 rounded-2xl border transition-colors focus-within:border-opacity-80", inputBg)}>
          <input
            type="text"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Ask a question..."
            className={clsx("flex-1 bg-transparent text-sm outline-none",
              text,
              isDark ? "placeholder:text-[#4a5568]" : "placeholder:text-[#b0a090]"
            )}
            disabled={isLoading}
          />
          <button
            ...
            className={clsx(
              ...
              input.trim() && !isLoading
                ? (isDark ? "bg-[#2979ff] text-white" : "bg-[#b5651d] text-white")
