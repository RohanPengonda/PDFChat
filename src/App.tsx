...
import {
  FileText, X, Menu, FileSearch, MessageSquare,
  Loader2, ClipboardList, Moon, Sun,
} from "lucide-react";
import { clsx } from "clsx";

interface Document { id: string; filename: string; original_name: string; }
...
export default function App() {
  ...
  const handleSummaryClick = async (doc: Document, e: React.MouseEvent) => {
    ...
    try {
      ...
      setSummary({ docName: doc.original_name, content: "Failed to generate summary." });
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className={clsx("px-4 py-3 flex items-center justify-between border-b", T.border)}>
        ...
        <span className={clsx("text-xs font-medium tracking-wide", T.subtext)}>ChatPDF Pro</span>
        <div className="flex items-center gap-1">
          <button onClick={() => setIsDark(!isDark)} className={clsx("p-1 rounded transition-colors", T.subtext)}>
            {isDark ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
          </button>
          <button onClick={() => setSidebarOpen(false)} className={clsx("lg:hidden p-1", T.subtext)}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        {/* Model selector */}
        <div className={clsx("flex items-center gap-2 px-3 py-2 rounded-lg border text-xs", T.modelBg, T.border)}>
          <span className={T.subtext}>✦</span>
          <select
            value={selectedModel}
            onChange={(e) => setSelectedModel(e.target.value)}
            className={clsx("flex-1 bg-transparent font-medium outline-none cursor-pointer", T.text)}
          >
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5 px-1">
            <p className={clsx("text-[10px] font-bold uppercase tracking-widest", T.label)}>Documents</p>
            {documents.length > 0 && (
              <button onClick={handleClearAll} className={clsx("text-[10px] font-medium", T.sourceLink)}>Clear All</button>
            )}
          </div>
          {documents.length === 0 ? (
            <p className={clsx("text-xs italic px-1 py-2", T.subtext)}>No documents yet</p>
          ) : (
            <ul className="space-y-0.5">
              {documents.map((doc) => (
                <li key={doc.id} className="group relative">
                  <button
                    onClick={() => { setSelectedDocId(doc.id); setPageNumber(1); setHighlightText(undefined); setMobileTab("pdf"); setSidebarOpen(false); }}
                    className={clsx("w-full text-left px-2 py-2 rounded-lg text-xs flex items-center gap-2 transition-all pr-14", selectedDocId === doc.id ? T.docSelected : T.docItem)}
                  >
                    <span className={clsx("text-[9px] font-bold px-1.5 py-0.5 rounded flex-shrink-0", T.docBadge)}>PDF</span>
                    <span className="truncate flex-1">{doc.original_name}</span>
                  </button>
                  <button
                    onClick={(e) => handleSummaryClick(doc, e)}
                    disabled={summaryLoadingId === doc.id}
                    className={clsx("absolute right-6 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1 rounded transition-all", T.sourceLink)}
                    title="Summary"
                  >
                    {summaryLoadingId === doc.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <ClipboardList className="w-3 h-3" />}
                  </button>
                  <button
                    onClick={() => handleDeleteDoc(doc.id)}
                    className={clsx("absolute right-1.5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1 rounded transition-all", T.subtext, "hover:text-red-400")}
                    title="Delete"
                  >
                    <X className="w-3 h-3" />
                  </button>
  ...
  return (
    <div className={clsx("flex h-screen overflow-hidden font-sans", T.app)}>
      {summary && <SummaryModal docName={summary.docName} content={summary.content} isDark={isDark} onClose={() => setSummary(null)} />}

      {/* ── DESKTOP sidebar (lg+) ── */}
      <div className={clsx("hidden lg:flex flex-col w-[240px] flex-shrink-0 border-r", T.sidebar, T.sidebarBorder)}>
        <SidebarContent />
      </div>
      ...
      {sidebarOpen && (
        <>
          ...
          <div className={clsx("lg:hidden fixed left-0 top-0 h-full w-[240px] z-40 flex flex-col border-r shadow-2xl", T.sidebar, T.sidebarBorder)}>
            <SidebarContent />
          </div>
        </>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="hidden lg:flex flex-1 overflow-hidden">
          ...
          {/* Chat Panel */}
          <div className={clsx("w-[400px] flex-shrink-0 border-l flex flex-col", T.sidebar, T.sidebarBorder)}>
            {chatPanel}
          </div>
        </div>
        ...
        <div className="hidden md:flex lg:hidden flex-col h-full">
          <div className={clsx("flex items-center justify-between px-4 py-3 border-b flex-shrink-0", T.sidebar, T.sidebarBorder)}>
            <button onClick={() => setSidebarOpen(true)} className={clsx("p-1.5 rounded-lg", T.subtext)}>
              <Menu className="w-5 h-5" />
            </button>
            <span className={clsx("text-sm font-semibold", T.text)}>ChatPDF Pro</span>
            <button onClick={() => setIsDark(!isDark)} className={clsx("p-1.5 rounded-lg", T.subtext)}>
              {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
          </div>
          <div className="flex flex-1 overflow-hidden">
            <div className={clsx("flex-1 overflow-hidden", T.chat)}>{chatPanel}</div>
            <div className={clsx("w-[320px] flex-shrink-0 border-l", T.sidebarBorder)}>{pdfPanel}</div>
          </div>
        </div>
        ...
        <div className="flex md:hidden flex-col h-full">
          ...
          <div className={clsx("flex-1 overflow-hidden", T.chat)}>
            {mobileTab === "sidebar" && <div className={clsx("h-full overflow-y-auto", T.sidebar)}><SidebarContent /></div>}
            {mobileTab === "pdf" && pdfPanel}
            {mobileTab === "chat" && chatPanel}
          </div>
          <div className={clsx("flex-shrink-0 border-t flex", T.sidebar, T.sidebarBorder)}>
            {([
